...
{
    public class ObjectOrientatedNamedServiceProviderDemo
    {
        [Fact]
        public void Test_Temperature_Mapper_Directly()
        {
            var sut = ObjectOrientatedNamedServiceProvider.Get();

            var converterMapper = sut.GetRequiredService<KelvinConverterMapper>();
            var mappings = _testCases.Select(tc =>
                (tc.Input, tc.ExpectedResult, ActualResult: converterMapper(tc.TempScale).ToKelvins(tc.Input)));
        ...
        private static (string TempScale, decimal Input, decimal ExpectedResult)[] _testCases = _testCasesInitializer.Value;
        ...
        public class TemperatureCalculator
        {
            ...
            public decimal GetTemperatureInKelvins(string scale, decimal value) => _mapperLookup(scale).ToKelvins(value);
        }

        public delegate IKelvinMapper KelvinConverterMapper(string key);

        public interface IKelvinMapper
        {
            decimal ToKelvins(decimal value);
        }

        private static class ObjectOrientatedNamedServiceProvider
        {
